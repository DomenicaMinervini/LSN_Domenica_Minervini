...
void Media_C(int L, int M, int N, double mean[], double square[]){
   ...
   fstream f;
   f.open("guadagno_call.dat",ios::out);
   ...
   f.close();
   cout << "E' stato prodotto il file 'guadagno_call.dat'." << endl;
}
...
void Blocchi_P(int L, int N, double mean[], double square[]){
   ...
   double P; //Guadagno per l'opzione Call. Variabile da integrare.
   double sum; //Somma degli elementi in un blocco. 
...
void Media_P(int L, int M, int N, double mean[], double square[]){
   ...
   fstream f;
   f.open("guadagno_put.dat",ios::out);
   ...
   f.close();
   cout << "E' stato prodotto il file 'guadagno_put.dat'." << endl;
}
