...
void Blocchi_1s(int E, int L, int N, double mean[], double square[]){
   ...
   questa percentuale e' 1.23. Da come percentuale di accettazione 50.09% nel caso dello stato fondamentale. */
   
   int i,j,l; //Contatori.
   double range=1.23; //range per lo stato fondamentale, accettazione = 50.09%.
   ...
   fstream g;
   g.open("coordinate_1s_far.dat",ios::out);
   ...
   g.close();
   cout << "E' stato prodotto il file 'coordinate_1s_far.dat'." << endl;
   rnd.SaveSeed();   
}
...
void Media_1s(int L, int M, int N, double mean[], double square[]){
   ...
   fstream f;
   f.open("raggi_1s_far.dat",ios::out);
   ...
   f.close();
   cout << "E' stato prodotto il file 'raggi_1s_far.dat'." << endl;
   cout << "---------------------------------------------" << endl;
...
void Blocchi_2p(int E, int L, int N, double mean[], double square[]){
   ...
   /*Ho fatto il test sul valore ideale del range in cui estrarre i valori casuali, in unita' di raggio di Bohr la percentuale
   ideale di valori accettati deve essere del 50%. Ho fatto 10000 estrazioni. Il valore del range che si avvicina di piu' a
   questa percentuale e' 4.41. Da come percentuale di accettazione 50.12% nel caso del primo stato eccitato.*/ 
   
   int i,j,l; //Contatori.
   double range=4.41; //range per lo stato eccitato, accettazione =  50.726%.
   ...
   //PARTE 3: ALGORITMO DI METROPOLIS; SIMULAZIONE VERA E PROPRIA
   fstream g;
   g.open("coordinate_2p_far.dat",ios::out);
   ...
   g.close();
   cout << "E' stato prodotto il file 'coordinate_2p_far.dat'." << endl;
   rnd.SaveSeed();   
}
...
void Media_2p(int L, int M, int N, double mean[], double square[]){
   ...
   fstream f;
   f.open("raggi_2p_far.dat",ios::out);
   ...
   f.close();
   cout << "E' stato prodotto il file 'raggi_2p_far.dat'." << endl;
   cout << "---------------------------------------------" << endl;
