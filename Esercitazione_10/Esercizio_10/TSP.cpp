...
int main (int argc, char *argv[]){
   ...
   //2. comunica il risultato della funzione collettiva Reduce che calcola 
   //   il minimo finale assoluto di tutte le ricerche in parallelo.
   ...
   for(gen=1; gen<generations; gen++){ //gen = numero della generazione.
      ...
      for(j=elite; j<half_num+1; j++){ //Faccio half_num tentativi di crossover. I primi elite elementi sono assegnati con elitarismo.
         ...
         Crossover_population(way, population[pos_one], population[pos_two], population_new, cut, j);
      ...
      for(i=0; i<ten_num; i++){
        ...
        imt = rnd.Dim(num);  //Estrae dei numeri interi tra 1 e num-1, come le scelte possibili degli elementi della popolazione. 
                             //L'elemento 0-esimo, il migiore della popolazione, è escluso dalle mutazioni. Bene: lo conservo.
   ...
   MPI_Reduce(&my_best, &irecv, 1, MPI_DOUBLE, MPI_MIN, root, MPI_COMM_WORLD); 

   if(rank==root){ //Stampa il risultato della riduzione solo chi la svolge: il nodo 0.
      cout << endl;
   ...
   //Indirizzo della variabile che mando, quante sono, tipo, chi lo invia, gruppo dei destinatari.
   MPI_Bcast(&irecv, 1, MPI_DOUBLE, root, MPI_COMM_WORLD); 
...
void Crossover_population(vector <City> way, Trip father, Trip mother, vector <Trip>& population_new, int cut, int icr){
